...
class GenericModelCRUD< Type extends mongoose.Document > {
    ...
    async findDocumentById( docId : string, populateFields: string | string[] = "" ): Promise< Type > {
        ...
        if ( !foundDocument ) throw `Document with ID:'${ docId }' not found in '${ this.model.collection.name }'`
    ...
    async editDocument( docId: string, data: Type ): Promise< Type > {
        ...
        if ( !updatedDocument ) throw `Document with ID:'${ docId }' not found in '${ this.model.collection.name }'`
    ...
    async deleteDocument( docId: string ): Promise< Type > {
        ...
        if ( !deletedDocument ) throw `Document with ID:'${ docId }' not found in '${ this.model.collection.name }'`
        ...
        if ( refFields ) {
            ...
            refFields.forEach( fieldName => {
                ...
                if ( modelFields[ fieldName ] && modelFields[ fieldName ].ref ) {
                    ...
                    throw `Property '${ fieldName }' exists in model '${ this.model.collection.name }' but has no reference with another model`
                }
                else {
                    throw `Property '${ fieldName }' does not exists in model '${ this.model.collection.name }'`
                }
